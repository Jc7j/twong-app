        ...
        {selectedProperty.invoices.map((invoice) => (
          <>
            <li
              ...
              className="flex justify-between text-black font-medium border-b border-black text-sm pb-1"
              onClick={() => handleInvoiceClick(invoice)}
            >
              {formatDate(invoice.invoice_month)}

              {invoice.management_fee > 1 && (
                <svg
                  width="15"
                  height="15"
                  viewBox="0 0 15 15"
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              {invoice.management_fee && <span className='flex justify-between font-normal text-sm'><p>Property Management Fee</p> <p>${invoice.management_fee}</p></span>}
                </div>
                <hr />
